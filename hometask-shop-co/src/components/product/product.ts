  ...
  return `
    <div class="product-card">
      <div class="product-card__image-wrapper">
        <img src="${thumbnail}" alt="${title}" class="product-card__image" />
      </div>
      <p class="product-card__title">${title}</p>
      <div class="product-card__rating">
        <div class="product-card__stars">${renderStars(rating)}</div>
        <span class="product-card__rating-value">${rating.toFixed(1)}/5</span>
      </div>
      <p class="product-card__price">$${price.toFixed(0)}</p>
    </div>
  `;
};
